...
object PageOneStepConvertRate {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //(3464a22d,(UserVisitAction1(2018-10-09,75,3464a22d,9,2018-10-09 11:09:25,null,-1,-1,null,null,41,37,5),2,3,4...))

    // 最核心的一步，每个session的单跳页面切片的生成，以及页面流的匹配，算法
    val pageSplitRDD = generateAndMatchPageSplit(sc, sessionId2actionsRDD, taskParam)
    // 返回：(1_2, 1)，(1_2, 1),(3_4, 1), ..., (100_101, 1)
  ...
  def persistConvertRate(spark:SparkSession, taskid:String, convertRateMap:collection.Map[String, Double]) {
    ...
    // [(1_2, 0.1),(2_3, 0.97)]
    val convertRate: Iterable[PageSplitConvertRate] = convertRateMap.map(item => PageSplitConvertRate(taskid,item._1,item._2.toString))

    //Array( PageSplitConvertRate(
    val pageSplitConvertRateRDD = spark.sparkContext.makeRDD(convertRate.toArray)
  ...
  def computePageSplitConvertRate(taskParam:JSONObject, pageSplitPvMap:collection.Map[String, Long], startPagePv:Long):collection.Map[String, Double] = {
    ...
    // lastPageSplitPv：存储最新一次的页面PV数量  .起始值是起始页面的点击数
    ...
    val convertRateMap = new mutable.HashMap[String, Double]() // 封装结果

    // 通过for循环，获取目标页面流中的各个页面切片（pv）
    for(targetPage2Page <- targetPagePairs){
      // 先获取pageSplitPvMap中记录的当前targetPage的数量
      // pageSplitPvMap：(1_2, 102320), (3_4, 90021), ..., (100_101, 45789)
      val targetPageSplitPv = pageSplitPvMap(targetPage2Page).toDouble
      ...
      // 对targetPage和转化率进行存储
      convertRateMap.put(targetPage2Page, convertRate)
      // 将本次的targetPage作为下一次的lastPageSplitPv
      lastPageSplitPv = targetPageSplitPv
    }

    convertRateMap// [(1_2, 0.1),(2_3, 0.97)]
  }
  ...
  def getStartPagePv(taskParam:JSONObject, sessionid2actionsRDD:RDD[(String, Iterable[UserVisitAction])]) :Long = {
    ...
    // sessionId2actionsRDD (3464a22d,(UserVisitAction1(2018-10-09,75,3464a22d,9,2018-10-09 11:09:25,null,-1,-1,null,null,41,37,5),2,3,4...))
  ...
  def generateAndMatchPageSplit(sc:SparkContext, sessionId2actionsRDD:RDD[(String, Iterable[UserVisitAction])], taskParam:JSONObject ):RDD[(String, Int)] = {
    ...
    sessionId2actionsRDD.flatMap{ case (sessionId, userVisitActions) =>
      ...
      // 最后保留：(1_2,5_6,6_7)
      // 输出：(1_2, 1) (5_6, 1) (6_7, 1) (1_2, 1)  还不需要聚合
