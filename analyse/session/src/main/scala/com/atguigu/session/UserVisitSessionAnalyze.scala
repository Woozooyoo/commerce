...
object UserVisitSessionAnalyze {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 构建Spark上下文
    val sparkConf = new SparkConf().setAppName("SessionAnalyzer").setMaster("local[*]") //spark://hadoop102:7077

    // 创建Spark客户端
    val spark = SparkSession.builder().config(sparkConf).enableHiveSupport().getOrCreate()
    val sc = spark.sparkContext
    spark.sql("use spark")
    //sc.setCheckpointDir("./analyse/session/sessionChk")

    // 首先要从user_visit_action的Hive表中，查询出来指定日期范围内的行为数据 过滤了时间
    val actionRDD: RDD[UserVisitAction] = this.getActionRDDByDateRange(spark, taskParam)
    //(2018-10-09,75,3464a22d,9,2018-10-09 11:09:25,null,-1,-1,null,null,41,37,5)

    // 将用户行为信息转换为 K-V 结构
    val sessionId2actionRDD: RDD[(String, UserVisitAction)] = actionRDD.map(item => (item.session_id, item))
    //(3464a22d,UserVisitAction(2018-10-09,75,3464a22d,9,2018-10-09 11:09:25,null,-1,-1,null,null,41,37,5))
    // 将数据进行内存缓存
    ...
    //(ebd4262a32, sessionId=ebd4262a32|searchKeywords=国贸大厦,日本料理,温泉|clickCategoryIds=2|visitLength=3152|stepLength=7|startTime=2018-10-09 16:04:15|age=1|professional=professional94|city=city72|sex=male)
    ...
    //(186b8f,sessionId=186b8f|searchKeywords=太古商场,重庆辣子鸡|clickCategoryIds=66, 55, 58|visitLength=3509|stepLength=69|startTime=2018-10-09 01:00:21|age=34|professional=professional57|city=city5|sex=female)
    // 对数据进行内存缓存
    ...
    sessionId2detailRDD.take(3).foreach(println)
    //(652231f,UserVisitAction(2018-10-09,14,652231f,8,2018-10-09 22:33:12,null,12,83,null,null,null,null,5))
    // 对数据进行内存缓存
    sessionId2detailRDD.persist(StorageLevel.MEMORY_ONLY)

    //sessionId2detailRDD.checkpoint()
    ...
    // top10CategoryList=Array[(  CategorySortKey(clickCount, orderCount, payCount), "categoryId|clickCount|orderCount|payCount" )]
  ...
  /**
    * 业务功能四：获取Top10热门品类中的Top10活跃Session
    *
    */
  def getTop10Session(spark: SparkSession, taskid: String, top10CategoryList: Array[(CategorySortKey, String)], sessionid2ActionRDD: RDD[(String, UserVisitAction)]) {
    ...
    /* 可以优化为 filter(热门品类List.contains(品类))*/
    ...
    val top10SessionObjectRDD = top10CategorySessionCountsRDD.flatMap { case (categoryid, clicks) =>
      // 先排序，然后取前10   按 (categoryid, sessionid + "," + count) 的count排序 >是倒序排序
    ...
    // top10SessionRDD[(sid,sid)] join sessionid2ActionRDD[(sid, action]
    ...
    sessionDetailRDD.toDF().write
      .format("jdbc")
      .option("url", ConfigurationManager.config.getString(Constants.JDBC_URL))
      .option("dbtable", "top10_session_detail")
      .option("user", ConfigurationManager.config.getString(Constants.JDBC_USER))
      .option("password", ConfigurationManager.config.getString(Constants.JDBC_PASSWORD))
      .mode(SaveMode.Append)
      .save()
  ...
  def getTop10Category(spark: SparkSession, taskid: String, sessionid2detailRDD: RDD[(String, UserVisitAction)]): Array[(CategorySortKey, String)] = {
    ...
    val categoryidRDD: RDD[(Long, Long)] = sessionid2detailRDD.flatMap { case (sessionid, userVisitAction) =>
      ...
      if (userVisitAction.order_category_ids != null) {
        for (orderCategoryId <- userVisitAction.order_category_ids.split(","))
          categoryBuffer += ((orderCategoryId.toLong, orderCategoryId.toLong))
      }
      // 一个session中支付的商品ID集合
      ...
      categoryBuffer // flatMap 能让每个ListBuffer 压平成里面的一个个tuple2元素
    }

    // 对重复的categoryid进行去重
    // 得到了所有被点击、下单、支付的商品的不重复的品类
    ...
    // categoryid2countRDD[(categoryId, "categoryId|点击的次数|下单的次数|支付的次数")]
    ...
    // 第五步：将数据categoryid2countRDD[(categoryId, "点击的次数|下单的次数|支付的次数")] 映射成<CategorySortKey,info>格式的RDD，然后进行二次排序（降序）
    ...
    // sortKey2countRDD[(   CategorySortKey(clickCount, orderCount, payCount), "categoryId|clickCount|orderCount|payCount")]

    // 降序排序
    val sortedCategoryCountRDD = sortKey2countRDD.sortByKey(false)

    // 第六步：用take(10)取出top10热门品类，并写入MySQL
    // Array[(  CategorySortKey(clickCount, orderCount, payCount), "categoryId|clickCount|orderCount|payCount" )]
    val top10CategoryList: Array[(CategorySortKey, String)] = sortedCategoryCountRDD.take(10)

    val top10CategoryRDD = spark.sparkContext.makeRDD(top10CategoryList).map { case (categorySortKey, line) =>
      val categoryid = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_CATEGORY_ID).toLong
      val clickCount = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_CLICK_COUNT).toLong
      val orderCount = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_ORDER_COUNT).toLong
      val payCount = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_PAY_COUNT).toLong
    ...
    top10CategoryRDD.toDF().write
      .format("jdbc")
      .option("url", ConfigurationManager.config.getString(Constants.JDBC_URL))
      .option("dbtable", "top10_category2")
      .option("user", ConfigurationManager.config.getString(Constants.JDBC_USER))
      .option("password", ConfigurationManager.config.getString(Constants.JDBC_PASSWORD))
      .mode(SaveMode.Append)
      .save()
  ...
  /**
    * 业务需求二：随机抽取session 100个
    *
    */
  def randomExtractSession(spark: SparkSession, taskUUID: String, filteredSessionid2AggrInfoRDD: RDD[(String, String)], sessionid2detailRDD: RDD[(String, UserVisitAction)]) {
    //(bdc66f5,sessionid=bdc66f5|searchKeywords=新辣道鱼火锅,国贸大厦|clickCategoryIds=15, 19|visitLength=2677|stepLength=5|startTime=2018-10-09 22:12:14|age=24|professional=professional12|city=city82|sex=male)
    //(652231f,UserVisitAction(2018-10-09,14,652231f,8,2018-10-09 22:33:12,null,12,83,null,null,null,null,5))

    // 第一步，计算出每天每小时的session数量，获取<yyyy-MM-dd_HH,aggrInfo>格式的RDD
    val time2sessionidRDD: RDD[(String, String)] = filteredSessionid2AggrInfoRDD.map { case (sessionid, aggrInfo) =>
    ...
    // countMap  Map<yyyy-MM-dd_HH, count>
    val countMap: collection.Map[String, Long] = time2sessionidRDD.countByKey()

    // 第二步，使用按时间比例随机抽取算法，计算出每天每小时要抽取session的索引，
    // 将<yyyy-MM-dd_HH,count>格式的map，转换成<yyyy-MM-dd,<HH,count>>的格式
    // dateHourCountMap  Map<yyyy-MM-dd,Map<(HH1,count),(HH2,count),(HH3,count)>>
    ...
    /**
      ...
      * 遍历每个小时，填充Map<date,Map<(hour1,(3,5,20,102)),..>>
      *
      * @param hourExtractMap  主要用来存放生成的随机值
      * @param hourCountMap    每个小时的session总数
      * @param dayTotalSession 当天所有的seesion总数
      */
    def hourExtractMapFunc(hourExtractMap: mutable.HashMap[String, mutable.ListBuffer[Int]],
                           ...
                           dayTotalSession: Long) {

      for ((hour, count) <- hourCountMap) {
        // 计算每个小时的session数量$count，占据当天总session数量的比例，直接乘以每天要抽取的数量
        // 就可以计算出，当前小时需要抽取的session数量
        var hourExtractNumber = ((count / dayTotalSession.toDouble) * extractNumberPerDay).toInt
        // 需要的大于 当前小时所有的session数量, 需要的数量改为当前数量
    ...
    for ((date, hourCountMap) <- dateHourCountMap) {
      ...
      // 计算出这一天的session总数
      val dayTotalSession = hourCountMap.values.sum

      // dateHourExtractMap[天，[小时，小时index列表]]
      dateHourExtractMap.get(date) match {
        case None => dateHourExtractMap(date) = new mutable.HashMap[String, mutable.ListBuffer[Int]]()
          // 更新index
          hourExtractMapFunc(dateHourExtractMap(date), hourCountMap, dayTotalSession)
        case Some(hourExtractMap) => hourExtractMapFunc(hourExtractMap, hourCountMap, dayTotalSession)
      }
    }
    ...
    // 执行groupByKey算子，得到<yyyy-MM-dd_HH,(aggrInfo0,aggrInfo1,aggrInfo2)>
    ...
    val sessionRandomExtract: RDD[SessionRandomExtract] = time2sessionsRDD.flatMap { case (dateHour, items) =>
      ...
      for (sessionAggrInfo <- items) {
        // 如果 在dateHourExtractMap[天，[小时，小时index列表]]的 取随机位置的小时index列表中包含当前的index，则提取此sessionAggrInfo中的数据
        ...
        // index自增
        index += 1
        /*可以优化 当 index  大于当前小时的count数量的时候 break  但是好像又不对 遍历items的次数就是小时的count数量了 */
      }
  ...
  def calculateAndPersistAggrStat(spark: SparkSession, value: mutable.HashMap[String, Int], taskUUID: String) {
    /*报错 Exception in thread "main" java.util.NoSuchElementException: key not found: session_count 是因为累加器是懒加载的*/
    // 从Accumulator统计串中 获取所有符合条件的Session个数
  ...
  def aggregateBySession(spark: SparkSession, sessinoid2actionRDD: RDD[(String, UserVisitAction)]): RDD[(String, String)] = {
    ...
    val userid2PartAggrInfoRDD: RDD[(Long, String)] = sessionid2ActionsRDD.map { case (sessionid, userVisitActions) =>
      ...
      val clickCategoryIdsSet = new util.HashSet[String]()
      ...
      // userVisitActions同一个sessionId 聚合的value是很多的action的数组  遍历session所有的访问行为
    ...
    // 从user_info表查询所有用户数据，并映射成<userid,Row>的格式
    import spark.implicits._
    val userid2InfoRDD = spark.sql("select * from user_info").as[UserInfo].rdd
