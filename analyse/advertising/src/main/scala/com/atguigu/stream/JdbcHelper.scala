...
object AdUserClickCountDAO {
  ...
  def findCountGreaterThanNumber(number: Int) = {
    // 获取对象池单例对象
    val mySqlPool = CreateMySqlPool()
    // 从对象池中提取对象
    val client = mySqlPool.borrowObject()

    val sql = "SELECT userid FROM ad_user_click_count " +
      "WHERE clickCount >=? "

    val blackUserIDList = ListBuffer[Int]()
    val params = Array[Any](number)

    // 根据多个条件查询指定用户的点击量，将查询结果累加到clickCount中
    client.executeQuery(sql, params, new QueryCallback {
      override def process(rs: ResultSet): Unit = {
        if (rs.next()) {
          blackUserIDList.append(rs.getInt(1))
        }
      }
    })
    // 使用完成后将对象返回给对象池
...
object AdStatDAO {
  ...
  /*TODO 不行*/
  def updateBatchOnDuplicateKey(adStats: Array[AdStat]) {
    // 获取对象池单例对象
    val mySqlPool = CreateMySqlPool()
    // 从对象池中提取对象
    val client = mySqlPool.borrowObject()
    ...
    val insertOnUpdateParamsList = ArrayBuffer[Array[Any]]()
    for (adStat <- adStats) {
      insertOnUpdateParamsList += Array[Any](adStat.date, adStat.province, adStat.city, adStat.adid, adStat.clickCount, adStat.clickCount)
    }

    client.executeBatch(sql, insertOnUpdateParamsList.toArray)
